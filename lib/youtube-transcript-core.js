...
export async function fetchYouTubeTranscript(videoId, groqApiKey = '') {
...
export function getGroqApiKeyFromRequest(requestOrReq) {
    if (requestOrReq instanceof Request) {
        return String(
            requestOrReq.headers.get('x-groq-api-key')
            || requestOrReq.headers.get('X-Groq-Api-Key')
            || new URL(requestOrReq.url).searchParams.get('groqKey')
            || ''
        ).trim();
    }

    const headerKey = requestOrReq.headers?.['x-groq-api-key'] || requestOrReq.headers?.['X-Groq-Api-Key'];
    const queryKey = requestOrReq.query?.groqKey;
